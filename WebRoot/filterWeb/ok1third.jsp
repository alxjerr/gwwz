<%@ page language="java" pageEncoding="utf-8"%>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html>
  <head>
    <title>商户注册成功</title>
    <link type="text/css" rel="stylesheet" href="../css/style.css" />
  </head>
  
  <body>
   <iframe src="../web/top.jsp" width="100%" height="170" frameborder="0"></iframe>
    <table border="0" cellspacing="0" cellpadding="0" width='960px' align="center">
  <tr height="60">
    <td><font size="2" color="#999999">您现在所在位置：<a href="../" target="_top">网站首页</a>&gt; <a href="../web/loginOrRegister.jsp" target="_top">登陆/注册</a>&gt; 注册成功</font></td>
  </tr>
</table>
   <table align="center">
   		<tr>
   			<td><img src="../image/icon/ok.png" /></td><td>&nbsp;&nbsp;&nbsp;&nbsp;<font size="+5" color="#99CC00"><strong>注册成功，请等待审核</strong></font></td>
   		</tr>
   	</table>
   	<table style="margin-left: 650px"><tr><td><a href="../">返回首页</a></td></tr></table>
   	<br />
   <div id="footer"  style="height:60px; background-color:#369;clear:both; ">
  <p style="text-align: center;	padding-top:20px;	color:#FFF"> <a href="../servlet/About"  ><font color="#FFFFFF">网站简介</font></a>   <a href="../servlet/Contact" ><font color="#FFFFFF">联系我们</a></font> </p>
</div>
  </body>
</html>
