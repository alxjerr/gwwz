		...
		throws ServletException, IOException {
			...
			if(PublicToolCheckParam.checkNullAndEmpty(thirdId) && PublicToolCheckParam.checkNullAndEmpty(isCooled)){
					...
					if(PublicToolCheckParam.checkNullAndEmpty(isSearching)){
						...
						PublicToolShowThirdInfoList.searchThirdInfo(request, response, cp, key);
						request.setAttribute("key", key);
						request.setAttribute("isSearching", "yes");
					}
					else{
						PublicToolShowThirdInfoList.showThirdInfoList(request, response, cp);
					}
			}
			request.getRequestDispatcher("../admin/manageThirdInfo.jsp").forward(request, response);
